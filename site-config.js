...
const SITE = {
  ...
  instagram: 'https://www.instagram.com/llancaaa/?hl=es-la',
  reddit:    'https://www.reddit.com/user/Ey40_0/',
  x:         'https://x.com/como_m_llmo',
};
...
const _ICONS = {
  ...
  x: `<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.253 5.622 5.912-5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>`,
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* data-site="footer-socials" → inyecta los iconos de redes sociales
   * Agregar <div data-site="footer-socials"></div> en cualquier footer
   * para que aparezcan automaticamente. */
  document.querySelectorAll('[data-site="footer-socials"]').forEach(el => {
    el.className = 'footer-socials';
    el.innerHTML = `
      <a href="${SITE.github}"    target="_blank" rel="noopener" aria-label="GitHub">${_ICONS.github}</a>
      <a href="${SITE.instagram}" target="_blank" rel="noopener" aria-label="Instagram">${_ICONS.instagram}</a>
      <a href="${SITE.reddit}"    target="_blank" rel="noopener" aria-label="Reddit">${_ICONS.reddit}</a>
      <a href="${SITE.x}"        target="_blank" rel="noopener" aria-label="X (Twitter)">${_ICONS.x}</a>
    `;
  });
});
